...
problems/dynamic.py
===================
**Mechanical / dynamic** one dimensional bar in traction problem.
...
def run_problem(
    ...
    if verbose:
        print_mesh_info(
            domain, V_u, V_alpha, label=f"MECH {physics}", comm_=domain.comm
        )
    ...
    fracture_e_density = fracture_energy_density(
        alpha, grad_sq(alpha, physics), l_hat_c, model_name
    )
    ...
    # -------------------------------------------------------------------------
    # Quasi-static loop -- while t < t_max
    # -------------------------------------------------------------------------
    u.x.array[:] = 0.0
    v.x.array[:] = 0.0
    a.x.array[:] = 0.0
    ...
    while t_cur + 1e-12 < t_max:
        ...
        n_alt = alt_min_loop(
            ...
            alpha_old_iter,
            error_L2_alpha_form,
            AltMin_parameters["max_iter"],
            AltMin_parameters["tol"],
            comm_=domain.comm,
        )
        ...
        qs["F"].append(
            domain.comm.allreduce(fem.assemble_scalar(reaction_right_form), op=MPI.SUM)
        )
        qs["P_el"].append(
            domain.comm.allreduce(fem.assemble_scalar(strain_energy_form), op=MPI.SUM)
        )
        qs["S"].append(
            domain.comm.allreduce(fem.assemble_scalar(fracture_energy_form), op=MPI.SUM)
        )
        qs["total"].append(qs["P_el"][-1] + qs["S"][-1])
    ...
    def u_newmark(u_, v_, a_, a_new_, dt):
        return (
            u_
            + dt * v_
            + 0.5 * dt**2 * ((1.0 - 2.0 * beta_v) * a_ + 2.0 * beta_v * a_new_)
        )
    ...
    while t_cur + 1e-12 < t_max:
        ...
        u_new.x.array[:] = (
            ...
            + 0.5
            * dt**2
            * ((1.0 - 2.0 * beta_v) * a.x.array + 2.0 * beta_v * a_new.x.array)
        )
        v_new.x.array[:] = v.x.array + dt * (
            (1.0 - gamma_v) * a.x.array + gamma_v * a_new.x.array
        )
        fem.set_bc(u_new.x.petsc_vec, bcs_u)
        fem.set_bc(v_new.x.petsc_vec, bcs_v)
        u_new.x.petsc_vec.ghostUpdate(
            addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD
        )
        v_new.x.petsc_vec.ghostUpdate(
            addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD
        )
        ...
        dyn["K"].append(
            domain.comm.allreduce(fem.assemble_scalar(kinetic_energy_form), op=MPI.SUM)
        )
        dyn["P_el"].append(
            domain.comm.allreduce(fem.assemble_scalar(strain_energy_form), op=MPI.SUM)
        )
        ...
        dyn["total"].append(dyn["K"][-1] + dyn["P_el"][-1] + dyn["S"][-1])

        a_max = _alpha_max(alpha, domain.comm)
        pbar_dyn.update(1)
        pbar_dyn.set_postfix(
            t=f"{t_cur:.3f}",
            U=f"{float(u_right_val.value):.3f}",
            K=f"{dyn['K'][-1]:.2e}",
            a_max=f"{a_max:.3f}",
        ...
        if paraview and (step % snap_every == 0):
