...
def plot_mechanical_run(
    ...
    ax_force.plot(
        dyn["U"], dyn["F"], "rx-", lw=0.6, alpha=0.7, label=rf"dyn, $\eta={eta}$"
    )
    ...
    if ph == "1D":
        ...
        if tri is not None:
            tpc = ax_dam.tripcolor(
                tri, alpha_dyn_final, cmap="inferno", shading="gouraud", vmin=0, vmax=1
            )
            fig.colorbar(tpc, ax=ax_dam, fraction=0.046, pad=0.04)
        ax_dam.set_xlabel(r"$\hat x$")
        ax_dam.set_ylabel(r"$\hat y$")
    ...
    ax_energy.plot(
        dyn["U"], dyn["K"], "mx-", lw=0.6, alpha=0.7, label=r"$\hat K$ (dyn only)"
    )
    ax_energy.plot(
        qs["U"], qs["P_el"], "b.-", lw=0.6, alpha=0.7, label=r"$\hat P_{el}$ QS"
    )
    ax_energy.plot(
        dyn["U"], dyn["P_el"], "bx-", lw=0.6, alpha=0.7, label=r"$\hat P_{el}$ Dyn"
    )
    ...
    ax_energy.plot(
        dyn["U"], dyn["total"], "kx-", lw=0.6, alpha=0.7, label="Total Dyn (incl K)"
    )
...
def plot_thermal_run(
    ...
    ax_force.plot(
        dyn["theta"], dyn["sigma_bar"], "rx-", lw=0.6, alpha=0.7, label="dynamic"
    )
    ...
    ax_energy.plot(
        dyn["theta"], dyn["K"], "mx-", lw=0.6, alpha=0.7, label=r"$\hat K$ (dyn)"
    )
    ax_energy.plot(
        qs["theta"], qs["P_el"], "b.-", lw=0.6, alpha=0.7, label=r"$\hat P_{el}$ QS"
    )
    ax_energy.plot(
        dyn["theta"], dyn["P_el"], "bx-", lw=0.6, alpha=0.7, label=r"$\hat P_{el}$ Dyn"
    )
    ax_energy.plot(
        qs["theta"], qs["P_f"], "g.-", lw=0.6, alpha=0.7, label=r"$\hat P_f$ QS"
    )
    ax_energy.plot(
        dyn["theta"], dyn["P_f"], "gx-", lw=0.6, alpha=0.7, label=r"$\hat P_f$ Dyn"
    )
    ax_energy.plot(qs["theta"], qs["S"], "r.-", lw=0.6, alpha=0.7, label=r"$\hat S$ QS")
    ax_energy.plot(
        dyn["theta"], dyn["S"], "rx-", lw=0.6, alpha=0.7, label=r"$\hat S$ Dyn"
    )
    ax_energy.plot(qs["theta"], qs["total"], "k.-", lw=0.6, alpha=0.7, label="Total QS")
    ax_energy.plot(
        dyn["theta"], dyn["total"], "kx-", lw=0.6, alpha=0.7, label="Total Dyn"
    )
    ...
    if ph == "1D":
        ...
        if tri is not None:
            ax_frag_qs.tripcolor(
                tri, last_qs, cmap="inferno", shading="gouraud", vmin=0, vmax=1
            )
            ax_frag_dyn.tripcolor(
                tri, last_dyn, cmap="inferno", shading="gouraud", vmin=0, vmax=1
            )
        for ax in (ax_frag_qs, ax_frag_dyn):
            ax.set_aspect("equal")
            ax.set_xlabel(r"$\hat x$")
